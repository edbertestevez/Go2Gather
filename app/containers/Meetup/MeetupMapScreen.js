...
class MeetupMapScreen extends Component {
	...
	componentWillMount(){
		...
		firebase.database().ref("/meetups/"+state.params.info.key+"/users").on('child_added', (snapshot) =>{
			if(snapshot.key != this.props.state.account.uid){
...
export default connect(mapStateToProps,mapDispatchToProps)(MeetupMapScreen);